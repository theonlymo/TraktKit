...
extension TraktManager {
     ...
     */
    @discardableResult
    public func getWatchlist(watchType: WatchedType, pagination: Pagination? = nil, sort: String? = nil, extended: [ExtendedType] = [.Min], completion: @escaping WatchlistCompletionHandler) -> URLSessionDataTask? {
        var query: [String: String] = ["extended": extended.queryString()]
        // pagination
        if let pagination = pagination {
            for (key, value) in pagination.value() {
        ...
        guard let request = try? mutableRequest(forPath: "sync/watchlist/\(watchType.rawValue)/\(sort ?? "")",
                                           withQuery: query,
                                           isAuthorized: true,
                                           withHTTPMethod: .GET) else { return nil }
        return performRequest(request: request, completion: completion)
    ...
    public func addToWatchlist(movies: [SyncId]? = nil, shows: [SyncId]? = nil, seasons: [SyncId]? = nil, episodes: [SyncId]? = nil, completion: @escaping ObjectCompletionHandler<WatchlistItemPostResult>) throws -> URLSessionDataTask? {
        let body = TraktMediaBody(movies: movies, shows: shows, seasons: seasons, episodes: episodes)
        guard let request = try? post("sync/watchlist", body: body) else { completion(.error(error: nil)); return nil }
        return performRequest(request: request, completion: completion)
    }
    ...
    public func removeFromWatchlist(movies: [SyncId]? = nil, shows: [SyncId]? = nil, seasons: [SyncId]? = nil, episodes: [SyncId]? = nil, completion: @escaping ObjectCompletionHandler<RemoveFromWatchlistResult>) throws -> URLSessionDataTask? {
        let body = TraktMediaBody(movies: movies, shows: shows, seasons: seasons, episodes: episodes)
        guard let request = try? post("sync/watchlist/remove", body: body) else { completion(.error(error: nil)); return nil }
        return performRequest(request: request, completion: completion)
    }
     ...
     */
    @discardableResult
    public func getFavorites(watchType: WatchedType? = nil, sort: String?, completion: @escaping WatchlistCompletionHandler) -> URLSessionDataTask? {
        guard let request = try? mutableRequest(forPath: "sync/favorites/\(watchType?.rawValue ?? "")/\(sort ?? "")",
                                           withQuery: [:],
                                           isAuthorized: true,
                                           withHTTPMethod: .GET) else { return nil }
        return performRequest(request: request, completion: completion)
    ...
    public func addToFavorites(
        ...
        seasons: [SyncId]? = nil,
        episodes: [SyncId]? = nil,
        completion: @escaping ObjectCompletionHandler<FavoritesItemPostResult>
    ) throws -> URLSessionDataTask? {
        let body = TraktMediaBody(movies: movies, shows: shows, seasons: seasons, episodes: episodes)
        guard let request = try? post("sync/favorites", body: body) else { completion(.error(error: nil)); return nil }
        return performRequest(request: request, completion: completion)
    }
    ...
    public func removeFromFavorites(
        ...
        seasons: [SyncId]? = nil,
        episodes: [SyncId]? = nil,
        completion: @escaping ObjectCompletionHandler<RemoveFavoritesItemResult>
    ) throws -> URLSessionDataTask? {
        let body = TraktMediaBody(movies: movies, shows: shows, seasons: seasons, episodes: episodes)
        guard let request = try? post("sync/favorites/remove", body: body) else { completion(.error(error: nil)); return nil }
        return performRequest(request: request, completion: completion)
    }
}
